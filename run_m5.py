...
try:
    runner = IterateModel(m5_r_s.MyModel, cond)
    model = runner.run()
    ...
    simname = 'm5-p{:.2f}-yr{:.2f}-ys{:.2f}'.format(cond['p_hi'], cond['molefrac_rcvr'], cond['molefrac_stor'])

    # Export result
    model.export('m5/'+simname)

    # Export log
    runner.export('m5/'+simname+'-log')
...
for i in com:
    if iterable(com[i]) and 'Th' in com[i]:
        ...
        plt.ylim(miny,maxy)
        plt.grid(True)
        plt.savefig('output/m5/m5-pinch_' + str(i) + '.png')
        plt.close()
