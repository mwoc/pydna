...
def _fluidextention():
    ...
    _fldext = {}
    _fldext[_fpath + 'fluids/'] = [(each[:-4].lower()) for each in os.listdir(_fpath + 'fluids/') if fnmatch.fnmatch(each, '*.fld')]
    _fldext[_fpath + 'fluids/'].extend([each for each in os.listdir(_fpath + 'fluids/') if fnmatch.fnmatch(each, '*.ppf')])
    _fldext[_fpath + 'mixtures/'] = [(each[:-4].lower()) for each in os.listdir(_fpath + 'mixtures/') if fnmatch.fnmatch (each, '*.mix')]
    return _fldext
...
def setup(hrf, *hfld, hfmix='hmx.bnc'):
    ...
    for each in hfld:
        if each.__class__ is list:
            for other in each:
                listhfld.append(other.lower())
        elif each.__class__ is str:
            listhfld.append(each.lower())
    ...
    for each in listhfld:
        ...
        else: raise RefpropinputError('fluid ' + each + ' not found' )
